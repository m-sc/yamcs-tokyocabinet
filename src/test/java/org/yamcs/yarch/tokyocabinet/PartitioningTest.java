...
public class PartitioningTest extends YarchTestCase {
    ...
    public void testIndexPartitioning() throws ParseException, StreamSqlException, IOException, InterruptedException {
        ydb.execute("create table test1(gentime timestamp, apidSeqCount int, primary key(gentime,apidSeqCount)) engine tokyocabinet partition by time(gentime('YYYY/DOY'))");
    ...
    public void testDoublePartitioning() throws Exception {
        ...
        ydb.execute("create table testdp(gentime timestamp, seqNumber int, part String, packet binary, primary key(gentime,seqNumber)) engine tokyocabinet partition by time_and_value(gentime('YYYY/DOY'),part) ");
